function [X]=ltsplit(lt1,lt2,varargin)
%ltsplit get the component of a group layer_tensors 
%  ltspit function split a group layer_tensors.
%  
%  ltsplit(lt1,lt2[,ltN],'kron');
%  lt1 and ltN are layer_tensor. If one is double array, it will be convent
%  to layer_tensor. Function return cell array.
%
%  ltsplit(lt1,lt2[,ltN],'outer');
%
%  Example:
%    A=mat2gray(imread('circuit.tif'));%280*272
%    ltcore=gqtt(A,[140,136;2,2],1e-8,'sym');
%    X=ltsplit(ltcore{1},ltcore{2},'kron'); 
%    imshow(mat2gray(X{2}))
%
%  see also gqtt
